  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <img src="/CnLogo2.png" alt="Logo" className="h-6 min-w-[4.5rem] min-h-[2.5rem] object-contain shrink-0 grow-0" />
        </motion.div>

        {/* Desktop Navigation */}
        <nav className="hidden md:flex space-x-2 text-sm font-medium px-4">
          {["home", "services", "approach", "benefits", "projects", "contact",].map((id) => (
            <motion.button
              key={id}
      ...
      <AnimatePresence>
        {isMenuOpen && (
          ...
          >
            {["home", "approach", "services", "benefits", "projects", "contact"].map((id) => (
              <motion.button
                key={id}
                onClick={() => scrollToSection(id)}
